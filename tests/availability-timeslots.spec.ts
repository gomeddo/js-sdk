import { DimensionAvailabilityTimeslots, SlotType } from '../src/availability-timeslots'
import { getResponse, getSlot } from './__utils__/availability-responses'

test('It parses the slots into timelines', async () => {
  const result = new DimensionAvailabilityTimeslots(
    getResponse(['Id 1'], [
  ...
  expect(result.timeSlots[0].type).toBe(SlotType.CLOSED)
  ...
  expect(result.timeSlots[1].type).toBe(SlotType.RESERVATION)
  ...
  expect(result.timeSlots[2].type).toBe(SlotType.OPEN)
  ...
  expect(result.timeSlots[3].type).toBe(SlotType.CLOSED)
